 ...
 *
 * Supports hex, rgb/rgba, hsl/hsla, and transparent values. Unsupported input
...
/**
 * Linearly interpolates between two colors in JS/worklet-safe code.
 *
...
function jsInterpolateColor(val: number, start: number, end: number, startColor: string, endColor: string) {
  ...
  /**
   * Parses a color string into RGBA numeric channels.
   *
 ...
 *
 * When transitioning to/from transparent, this avoids dark/gray artifacts by
 * borrowing the non-transparent RGB channels and only fading alpha.
 *
 * @param val - Driver value.
...
function smartInterpolateColor(val: number, input: number[], output: string[]) {
  ...
  /**
   * Rewrites transparent color to share the source RGB with alpha 0.
   *
   * @param transp - Candidate transparent color.
   * @param source - Source color to borrow RGB from.
   * @returns RGBA string suitable for smooth alpha interpolation.
   */
 ...
 *
 * This allows segment lookup once per frame and reuse across many properties.
 *
 * @param val - Current axis value.
 * @param input - Sorted axis anchors.
 * @returns Segment index/progress info and whether the value is clamped to a constant edge.
 */
...
export default function Aniview(props: AniviewProps) {
  ...
  const baked = useMemo((): BakedResult | null => {
    ...
    /**
     * Determines whether a style key is color-like and should use color interpolation.
     *
    ...
    try {
      ...
      if (frames) {
        ...
        /**
         * Scans a frame and registers any missing baseline properties in `homeProps`.
         *
         * @param frame - Spatial or event frame definition.
         * @returns void
         */
       ...
       *
       * @param tAxis - Traversed axis values.
       * @param fAxis - Fixed axis values.
       * @param isH - Whether the lane is horizontally traversed.
       * @param keys - Property keys to bake.
       * @param isColor - Whether values are color strings.
       * @returns Baked lane descriptors containing output ranges and const flags.
       */
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    /**
     * Resolves neighboring lane rows and mix factor for a camera position.
     *
     * @param lanes - Baked lane rows.
     * @param pos - Camera position on the fixed axis.
     * @returns Neighbor lanes, blend factor, and nearest distance, or null when lanes are unavailable.
     */
    ...
    /**
     * Computes spatial interpolation values for a key set and writes into `spatialVals`.
     *
     * @param keys - Property keys to interpolate.
     * @param baked_H - Horizontal lane data.
     * @param baked_V - Vertical lane data.
     * @param isColor - Whether values are color strings.
     * @returns void
     */
    ...
    (Object.entries(baked.eventLanes) as [string, BakedLane][]).forEach(([eName, lane]) => {
      ...
      const laneWeight = lane.eventPersistent ? 1 : presence; // presence is ~1 on home page
    ...
    // 3. Composition Phase
    /**
     * Composes event contribution over spatial baseline for numeric properties.
     *
     * @param k - Property key.
     * @param base - Spatially interpolated base value.
     * @returns Final numeric value with event offsets/modulation applied.
     */
